...
class Trader:
    ...
    def compute_orders_c_and_cc(self, order_depths):
        ...
        res = mid_price['COCONUT_COUPON'] - coef*mid_price['COCONUT']
        logger.print(f'Residual std Nuts: {std}')
        trade_at = std*1
        close_at = std*(-0.5)

        coco_pos = self.position['COCONUT']
        coco_neg = self.position['COCONUT']

        if res > trade_at:
            vol = self.position['COCONUT_COUPON'] + self.POSITION_LIMIT['COCONUT_COUPON']
            if vol > 0:
        ...
        elif res < close_at and self.position['COCONUT_COUPON'] < 0:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if 'COCONUT' in state.order_depths.keys() and 'COCONUT_COUPON' in state.order_depths.keys():
            coupon_orders = self.compute_orders_c_and_cc(state.order_depths)
