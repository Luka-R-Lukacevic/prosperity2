...
class Trader:
    ...
    def compute_orders_c_and_cc(self, order_depths):
        ...
        if coco_vol_to_trade > 0:
            orders['COCONUT'].append(Order('COCONUT', best_sell['COCONUT'], coco_vol_to_trade))
        elif coco_vol_to_trade < 0:
            orders['COCONUT'].append(Order('COCONUT', best_buy['COCONUT'], coco_vol_to_trade))
    
        if coco_coupon_vol_to_trade > 0:
            orders['COCONUT_COUPON'].append(Order('COCONUT_COUPON', best_sell['COCONUT_COUPON'], coco_coupon_vol_to_trade))
        elif coco_coupon_vol_to_trade < 0:
            orders['COCONUT_COUPON'].append(Order('COCONUT_COUPON', best_buy['COCONUT_COUPON'], coco_coupon_vol_to_trade))
        logger.print(orders['COCONUT'])
        return orders
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if 'COCONUT' in state.order_depths.keys() and 'COCONUT_COUPON' in state.order_depths.keys():
            orders_nuts = self.compute_orders_c_and_cc(state.order_depths)
            orders_result['COCONUT_COUPON'] += orders_nuts['COCONUT_COUPON']
            orders_result['COCONUT'] += orders_nuts['COCONUT']
