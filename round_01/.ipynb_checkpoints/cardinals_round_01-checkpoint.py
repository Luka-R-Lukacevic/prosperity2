...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if n == 10:
            starfruit_lb = self.calc_next_price_starfruit(new_starfruit_cache, state)-1
            starfruit_ub = self.calc_next_price_starfruit(new_starfruit_cache, state)+1
        else:
            if n > 0:
                starfruit_lb = (bs_starfruit+bb_starfruit)/2 - 1  #for the first 10 timestamps we use the current mid price as our price prediction
                starfruit_ub = (bs_starfruit+bb_starfruit)/2 + 1
